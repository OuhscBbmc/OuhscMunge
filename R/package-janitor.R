...
package_janitor_remote <- function(
  ...
  pak::pkg_install(ds_install_from_cran$package_name, lib = lib, ask = FALSE)
  ...
  #     pak::pkg_install(to_install, lib = lib, ask = FALSE)
  # }
  ...
  repository_names <- paste0("github::", ds_install_from_github$github_username, "/", ds_install_from_github$package_name)
  if (1L <= length(repository_names))
    pak::pkg_install(repository_names, lib = lib, ask = FALSE)
