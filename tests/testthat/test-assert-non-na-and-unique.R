...
test_that("assert_non_na_and_unique -all - fails", {
  expect_error( assert_non_na_and_unique(c(1, 1:100                                                                                 ) , "integer"   ))
  expect_error( assert_non_na_and_unique(c(.2, .2, runif(100)                                                                       ) , "numeric"   ))
  expect_error( assert_non_na_and_unique(c("a", letters                                                                             ) , "character" ))
  expect_error( assert_non_na_and_unique(c(T, T, F                                                                                  ) , "logical"   ))
  expect_error( assert_non_na_and_unique(c(as.Date("2017-04-30"), seq.Date(as.Date("2015-01-02"), as.Date("2017-04-30"), by="days") ) , "Date"      ))
})
